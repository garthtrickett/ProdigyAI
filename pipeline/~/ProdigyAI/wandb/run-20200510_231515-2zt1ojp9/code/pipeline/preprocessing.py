...
volume_min = wandb.config['params']['minimum_return']['value'] + wandb.config[
    'params']['vol_min_modifier']['value']
...
use_sample_weights = wandb.config['params']['use_fractional_differentiation']['value'] 
...
if regenerate_features_and_labels == True:
    ...
    vertical_barrier_timestamps = ml.labeling.add_vertical_barrier(
        ...
        close=data["close"],
        num_seconds=vertical_barrier_seconds['value'])
    end = time.time()
    ...
    triple_barrier_events = ml.labeling.get_events(
        ...
        min_ret=wandb.config['params']['minimum_return']['value'],
        num_threads=cpus * 2,
