...
def get_labels(close, events, min_ret=0, sign_label=True, zero_label=0):
    ...
    out["ret"] = close.loc[events["t1"].values].values / close.loc[events.index] - 1.0
    ...
    if "side" in events:
        ...
        out.loc[
            (out["ret"] <= min_ret) & (out["ret"] >= -min_ret), "label"
        ] = zero_label
    if not sign_label:
...
def get_barrier_labels(
    ...
    labels = get_labels(
        close, events, min_ret=min_ret, sign_label=sign_label, zero_label=zero_label
    )
    return labels
