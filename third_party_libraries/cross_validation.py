...
class BaseTimeSeriesCrossValidator:
    ...
    def __init__(self, n_splits=10):
        if not isinstance(n_splits, numbers.Integral):
            raise ValueError(
                f"The number of folds must be of Integral type. {n_splits} of type {type(n_splits)}"
                f" was passed."
            )
        n_splits = int(n_splits)
        if n_splits <= 1:
            raise ValueError(
                f"K-fold cross-validation requires at least one train/test split by setting n_splits = 2 "
...
class PurgedWalkForwardCV(BaseTimeSeriesCrossValidator):
    ...
    def __init__(
        self, n_splits=10, n_test_splits=1, min_train_splits=2, max_train_splits=None
    ):
        super().__init__(n_splits)
        if not isinstance(n_test_splits, numbers.Integral):
            raise ValueError(
                f"The number of test folds must be of Integral type. {n_test_splits} of type "
                f"{type(n_test_splits)} was passed."
            )
        n_test_splits = int(n_test_splits)
        if n_test_splits <= 0 or n_test_splits >= self.n_splits - 1:
            raise ValueError(
                f"K-fold cross-validation requires at least one train/test split by setting "
        ...
        if not isinstance(min_train_splits, numbers.Integral):
            raise ValueError(
                f"The minimal number of train folds must be of Integral type. {min_train_splits} of type "
                f"{type(min_train_splits)} was passed."
            )
        min_train_splits = int(min_train_splits)
        if (
            min_train_splits <= 0
            or min_train_splits >= self.n_splits - self.n_test_splits
        ):
            raise ValueError(
                f"K-fold cross-validation requires at least one train/test split by setting "
        ...
        if not isinstance(max_train_splits, numbers.Integral):
            raise ValueError(
                f"The maximal number of train folds must be of Integral type. {max_train_splits} of type "
                f"{type(max_train_splits)} was passed."
            )
        max_train_splits = int(max_train_splits)
        if (
            max_train_splits <= 0
            or max_train_splits > self.n_splits - self.n_test_splits
        ):
            raise ValueError(
                f"K-fold cross-validation requires at least one train/test split by setting "
...
class CombPurgedKFoldCV(BaseTimeSeriesCrossValidator):
    ...
    def __init__(
        ...
        if not isinstance(embargo_td, pd.Timedelta):
            raise ValueError(
                f"The embargo time should be of type Pandas Timedelta. {embargo_td} of type "
...
def compute_fold_bounds(
    cv: BaseTimeSeriesCrossValidator, split_by_time: bool
) -> List[int]:
    """
    ...
    if split_by_time:
        ...
        fold_bounds_times = [
            cv.pred_times.iloc[0] + fold_time_span * n for n in range(cv.n_splits)
        ]
...
def embargo(
    ...
    # last_test_eval_time = cv.eval_times.iloc[test_indices[:test_fold_end]].max() # code before random user edit
    last_test_eval_time = cv.eval_times.iloc[test_indices[0] : test_fold_end].max()
...
# cv = CombPurgedKFoldCV(n_splits=4, n_test_splits=2, embargo_td=pd.Timedelta(minutes=0))
# count = 0
# for train_set, test_set in cv.split(
#     X=X, y=X.A, pred_times=pred_times, eval_times=exit_times
# ):
#     print(len(train_set))
#     print(len(test_set))

# Do hyperparameter tuning on train_set then use the best result on the test set
