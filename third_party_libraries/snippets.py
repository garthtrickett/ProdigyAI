...
def fracDiff(series, d, thres=0.01):
    ...
    for name in series.columns:
        ...
        for iloc in range(skip, seriesF.shape[0]):
            ...
            if not np.isfinite(series.loc[loc, name]).any():
...
def crossValPlot(skf, classifier, X, y):
    ...
    for train, test in skf.split(X, y):
        probas_ = classifier.fit(X.iloc[idx[train]], y.iloc[idx[train]]).predict_proba(
            X.iloc[idx[test]]
        )
    ...
    ax.plot([0, 1], [0, 1], linestyle="--", lw=2, color="r", label="Luck", alpha=0.8)
...
def testFunc(
    ...
    kargs = {
        "pathOut": PurePath(pdir / "testFunc").as_posix(),
        "n_estimators": n_estimators,
        "tag": "testFunc",
        "cv": cv,
    }
...
def plotFeatImportance(pathOut, imp, oob, oos, method, tag=0, simNum=0, **kargs):
    ...
    if method == "MDI":
        ...
        mpl.axvline(1.0 / imp.shape[0], lw=1.0, color="r", ls="dotted")
