...
def fitness(num_input_nodes, batch_size):
    ...
    blackbox = model.fit(
        x=X, y=y, epochs=1, batch_size=batch_size, validation_split=0.15
    )
...
#     func=fitness, dimensions=dimensions, n_calls=11, n_jobs=-1, x0=default_parameters
# )
...
def fitness(num_input_nodes, batch_size):
    ...
    # named blackbox becuase it represents the structure
    blackbox = model.fit(
        x=X_train, y=Y_train, epochs=1, batch_size=batch_size, validation_split=0.15
    )
    # return the validation accuracy for the last epoch.
